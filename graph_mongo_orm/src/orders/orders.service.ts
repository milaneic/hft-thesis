...
export class OrdersService {
  ...
  findOne(id: string) {
    const order = this.ordersRepo.findOne({ where: { _id: new ObjectId(id) } });
  ...
  async update(id: string, updateOrderInput: UpdateOrderInput) {
    try {
      ...
      if (updateOrderInput.tripId) order.tripId = updateOrderInput.tripId;
  ...
  async remove(id: string) {
    const res = await this.ordersRepo.delete({ _id: new ObjectId(id) });
  ...
  async countOrdersByTripId(tripId: string) {
    const count = (await this.ordersRepo.find({ where: { tripId } })).length;
    return count;
  }
